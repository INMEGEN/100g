...
java -Xmx4g -jar $GATKdir/GenomeAnalysisTK.jar -T VariantAnnotator -R $REF/hg19.fa --variant $i --dbsnp $VAR/dbsnp_138.hg19.vcf -L $i -o $iname.gatk.vcf --variant_index_type LINEAR --variant_index_parameter 128000
...
java -jar $SNPEFFdir/SnpSift.jar dbnsfp -v -db $SNPEFFconfig/dbNSFP/dbNSFP2.4.txt.gz $iname.ann.vcf > $iname.ann.cons.dbnsfp.vcf
